...
```
> **API Check**: 서버 실행 후 다음 주소에서 문서를 확인할 수 있습니다.
...
├── api/                 # 프레젠테이션 계층 (Router, Dependency Injection)
│   ├── deps.py          # 공통 의존성 (Auth Guard, DB Session)
│   └── v1/
│       ├── auth.py      # 인증 컨트롤러 (Thin Controller)
│       └── items.py     # 아이템 컨트롤러
├── core/                # 핵심 유틸리티, 설정, 예외 처리
├── services/            # 애플리케이션 계층 (비즈니스 로직)
│   ├── auth_service.py
│   └── item_service.py
├── domain/              # 도메인 계층 (순수 로직, 엔티티, 인터페이스)
│   ├── models/
│   │   ├── user.py      # User Entity & DTOs
│   │   └── item.py      # Item Entity & DTOs
│   └── repository_interfaces/
│       ├── user_repository.py
│       └── item_repository.py
└── infrastructure/      # 인프라 계층 (DB 구현체)
    └── repositories/
```
...
- [x] ~~**4. Migrations**: `Alembic` 스키마 관리. 구현 완료.~~
...
- [x] ~~**9. CI/CD**: GitHub Actions pipeline setup. 구현 완료.~~
- [x] ~~**9. CI/CD**: GitHub Actions pipeline setup. 구현 완료.~~
- [x] ~~**10. DB Security**: Role Separation (DDL vs DML). 구현 완료.~~
- [x] ~~**11. Dockerize**: Dockerfile & docker-compose setup. Render 배포 완료.~~

## 🚀 Deployment
배포는 **Render**를 통해 이루어지며, Docker 기반으로 실행됩니다.
자세한 설정 방법과 배포 파이프라인 구조는 [배포 가이드 (Deployment Guide)](docs/deployment-guide.md)를 참고하세요.
